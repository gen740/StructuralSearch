...
#include "autodiff/constant.hpp"
#include "autodiff/single_variable.hpp"

namespace Lattice::Autodiff {
...
struct InternalNum {
  ...
  [[nodiscard]] std::array<size_t, Order> get_repr_arr() const {
    return repr;
  }
  ...
  void normalize() {
    std::sort(repr.begin(), repr.end(), std::greater<>());
  }
};

template <size_t Deps = 2, size_t Order = 2>
class Variable {
 public:
  ...
  [[nodiscard]] constexpr Variable operator+(const Variable& rhs) const {
    Variable ret(rhs);
  ...
  constexpr void operator+=(const Variable& rhs) {
    for (size_t i = 0; i < rhs.repr.size(); i++) {
      this->repr[i] += rhs.repr[i];
    }
  }
  ...
  [[nodiscard]] constexpr Variable operator-(const Variable& rhs) const {
    Variable ret(*this);
    for (size_t i = 0; i < rhs.repr.size(); i++) {
      ret.repr[i] -= rhs.repr[i];
    }
    return ret;
  }

  [[nodiscard]] Variable operator*(const Variable& rhs) {
    Variable ret;
  ...
  [[nodiscard]] Variable operator*(arithmetic auto& rhs) {
    Variable ret{*this};
    for (auto&& i : ret.repr) {
      i *= rhs;
    }
  ...
  friend constexpr Variable inv(const Variable& other) {
    return other.inv();
  }
  ...
  friend constexpr Variable sin(const Variable& other) {
    return other.sin();
  }
  ...
  friend constexpr Variable cos(const Variable& other) {
    return other.cos();
  }
  ...
  friend constexpr Variable tan(const Variable& other) {
    return other.tan();
  }
  ...
  friend constexpr Variable exp(const Variable& other) {
    return other.exp();
  }
  ...
  friend constexpr Variable log(const Variable& other) {
    return other.log();
  }
  ...
  [[nodiscard]] constexpr Variable sqrt() const {
    return this->pow(1. / 2.);
  }

  friend constexpr Variable sqrt(const Variable& other) {
    return other.sqrt();
  }

  [[nodiscard]] constexpr Variable cbrt() const {
    return this->pow(1. / 3.);
  }

  friend constexpr Variable cbrt(const Variable& other) {
    return other.cbrt();
  }
